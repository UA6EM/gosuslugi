...
# В верхних версиях UBUNTU пакет Крипто-Про может требовать установки пакета motiff, ставим так:
sudo apt install motif*

# Если имя контейнера было сформировано из под WINDOWS и включает символы русской кодировки, то можем иметь при вводе команды:
 /opt/cprocsp/bin/amd64/csptest -keyset -enum_cont -verifycontext -fqcn
